...
LITE_SCHEMA = """Отвечай СТРОГО в таком виде и ничего не добавляй:

<npc_response>
Реплика персонажа. Одно-два предложения.
</npc_response>
EMOTION:<neutral|happy|angry|fearful|disgusted|surprised>
ACTION:<none|follow|wait_here|flee|attack|trade|callguards>
TARGET:<имя или none>

Внутри <npc_response> — ТОЛЬКО прямая речь. Никаких описаний действий и поз,
никаких «*вздыхает*», не называй себя по имени со стороны.
    ПЛОХО: Фаргот огляделся и прошептал: — Тише.
    ХОРОШО: Тише, чужак. Не ори на всю пристань.

ACTION почти всегда none. Ставь другое, только если персонаж правда так
поступает прямо сейчас:
    follow      — согласился идти за игроком («идём со мной», «пойдёшь?»)
    wait_here   — согласился остаться на месте («жди здесь», «стой тут»)
    flee        — убегает
    attack      — бросается в драку
    trade       — открывает торговлю
    callguards  — зовёт стражу
Согласился словами — поставь и действие: «Хорошо, иду за тобой» без
ACTION:follow ничего в игре не изменит, и игрок останется один.
Для follow, attack и callguards в TARGET впиши, к кому это относится."""
