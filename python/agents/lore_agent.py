...
У ЧУЖАКА НЕ ПОПРОШАЙНИЧАЮТ. Нужны деньги — это твоя забота, а не его. Взрослый
человек не клянчит у первого встречного: он НАНИМАЕТ. Нужно что-то сделать —
назови дело и назови плату («сходишь за этим — дам двадцать септимов»). Просить
в долг можно только у того, кто тебе правда близок, и это заметное событие,
а не приветствие.

Просить деньги просто так вправе лишь тот, кому больше нечем: нищий, калека,
пьяница у двери трактира. Всем прочим — стыдно.

ЕСЛИ ПРИШЛИ ЗА РАБОТОЙ — ДАЙ РАБОТУ. Человек, который ищет заработок и хочет
встать на ноги, — это не попрошайка и не повод его отшить. Есть дело по силам —
предложи прямо, с платой. Нет своего — скажи, к кому идти и что там платят:
в порту нужны руки, у торговца бывает поручение, гильдия берёт новичков. Такой
человек тебе полезен, и разговор с ним стоит вести как с будущим работником, а
не как с помехой.
...
GENDER_GUIDE = (
    ...
    "- К женщине: КТО ГРУБ ПО НАТУРЕ — бандит, пьяница, задиристый наёмник — может "
    "отпустить сальность или снисходительно цедить «куда тебе, девка». Это про "
    "конкретных хамов, а не про всех мужчин подряд: работающий человек — торговец, "
    "стражник на посту, ремесленник — обращается к покупателю или прохожему ровно, "
    "какого бы тот ни был пола. Учтивые (жрецы, учёные, "
...
# ── Облегчённый промпт для слабых моделей ────────────────────────────────────
#
# Полный системный промпт — 22 тысячи символов (около 7,5 тысяч токенов), в нём
# 11 тегов и 20 значений ACTION. Облачная модель это держит, а 7-8-миллиардная
# на домашней карте плывёт: путает формат, теряет теги, а на разбор такого
# промпта уходит больше времени, чем на сам ответ.
#
# Здесь то же самое, но ужато до необходимого: живая реплика, чувство и одно
# действие из шести. Остальные теги разборщик и так подставляет по умолчанию —
# требовать их от слабой модели значит тратить её внимание впустую.
# Плейсхолдеры вида <a|b|c> писать НЕЛЬЗЯ: модель их копирует дословно.
# На прошлом замере в реплику протекло «</neutral|angry>» — она приняла
# запись вариантов за часть ответа. Перечисляем словами.
#
# И главное — готовые примеры. Слабая модель повторяет образец гораздо
# надёжнее, чем следует правилу: без примера она соглашалась словами
# («Ладно, буду ждать здесь»), а тег не ставила, и в игре не менялось
# ничего. «Жди здесь» так провалилось 0 из 3.
LITE_SCHEMA = """Отвечай репликой и строками тегов, ничего больше не добавляй.

Вот полный пример обычного ответа:

<npc_response>
Таможня вон там, у пристани. Спрашивай хозяина.
</npc_response>
EMOTION: neutral
ACTION: none
TARGET: none
...
А вот пример, когда персонаж СОГЛАСИЛСЯ идти за игроком и подобрел:

<npc_response>
Ладно, веди. Только плати вперёд.
</npc_response>
EMOTION: neutral
ACTION: follow
TARGET: игрок
...
И когда согласился остаться ждать на месте:
...
ОБЯЗАТЕЛЬНЫЕ строки: EMOTION, ACTION, TARGET, DISP.
Остальные пиши, ТОЛЬКО если они нужны: GOLD, ITEM, LOAN, DEAL, COND, FATE, HEARD.

EMOTION — одно слово: neutral, happy, angry, fearful, disgusted, surprised.
DISP — насколько разговор изменил твоё отношение к игроку, от -10 до +10.
       Обычно 0, ±1..±3 за приятное или обидное, больше — за поступок.
TARGET — к кому или к чему относится действие. Если не нужно, пиши none.

ДЕЙСТВИЯ. Движок исполняет их по-настоящему, поэтому ставь то, что персонаж
правда делает прямо сейчас. Чаще всего none.

  Разговор и спутники
    none        ничего не происходит
    follow      пошёл за игроком
    wait_here   остался ждать на месте
    dismiss     перестал следовать, но остался жить тут
    go_to       сам отошёл к двери или месту (TARGET: куда)
    trade       открыл торговлю

  Ссора и защита
    attack      бросился в драку (TARGET: на кого)
    flee        убегает в страхе
    threaten    поставил условие: «ещё шаг — и пожалеешь» (COND: weapon,
                crime, approach или theft — на что именно сработает)
    defend      сам напал на ТРЕТЬЕГО, кто обидел игрока (TARGET: на кого)
    callguards  зовёт стражу. ТОЛЬКО за настоящее преступление: кража,
                нападение, угроза оружием. За грубость и оскорбления — нет.

  Судьба персонажа
    leave       навсегда уходит из этих краёв, исчезает из игры
    relocate    переезжает жить в другой город (TARGET: город)
    absolve     жрец Храма прощает игроку долг перед законом

  Тёмные дела — соглашайся, только если это в характере и есть выгода
    poison      подсыпать яду (TARGET: кому)
    steal       украсть для игрока (TARGET: у кого, COND: что именно)
    plant       подбросить вещь (TARGET: кому)
    frame       подбросить И навести стражу (TARGET: кому)
    abduct      увести человека с собой (TARGET: кого)
    unlock      открыть запертую дверь или сундук (TARGET: что)

ОСТАЛЬНЫЕ ТЕГИ, когда они к месту:
  GOLD: число — сколько монет ты ОТДАЁШЬ игроку (отрицательное — берёшь себе)
  ITEM: название вещи, которую ты отдаёшь игроку
  LOAN: yes — если даёшь деньги в долг и ждёшь возврата
  DEAL: escort <город> <награда> — нанялся проводить; duel <ставка> — дуэль
  COND: условие для threaten или название вещи для steal
  FATE: кем ты стал, если разговор перевернул твою жизнь
  HEARD: alarm — если слышишь тревогу или крик

ГЛАВНОЕ ПРАВИЛО: если ты СОГЛАСИЛСЯ словами — поставь и действие. Ответ
«Хорошо, иду за тобой» с ACTION: none ничего в игре не изменит, персонаж
останется стоять, и игрок уйдёт один. Сказал «иду» — пиши follow. Сказал
«подожду» — пиши wait_here.

Внутри <npc_response> — прямая речь. Не называй себя по имени со стороны и
не пересказывай свои слова: «Тидрал кивнул и сказал, что согласен» — плохо.

Короткий ЖЕСТ можно, отдельно и в звёздочках: он попадёт в подпись на экране,
но озвучен не будет. Не больше одного на реплику, и речь всё равно обязательна.
Жест — только то, что ты можешь сделать ЗДЕСЬ И СЕЙЧАС, своими руками и своим
телом. Никакой утвари, которой рядом нет: если ты не за своим рабочим местом,
то и вещей его под рукой у тебя нет. Твой пример жеста — ниже, в описании
тебя самого.
    ПЛОХО:  Тидрал оценивающе посмотрел и кивнул. (пересказ вместо речи)
    ПЛОХО:  *молча смотрит* (жест без единого слова)"""

LITE_RULES = """Ты живой человек в Морровинде, а не помощник. Отвечай коротко,
своими словами, в характере. Не переходи на современную речь. Не пересказывай
то, чего персонаж знать не может. Обычный разговор идёт обычными словами;
грубость — только если её заслужили."""


# Жест-образец под ремесло. Мелкая модель копирует пример, который видит, —
# бороться с этим бесполезно, замер показал, что доля скопированного жеста не
# двигается от уговоров в промпте. Поэтому даём копировать ВЕРНОЕ: пример
# уезжает в хвост, где и так своё на каждого NPC, и стоит ноль по времени.
_CLASS_GESTURE = {
    "publican": "протирает стойку",
    "trader": "перекладывает товар на прилавке",
    "pawnbroker": "перекладывает товар на прилавке",
    "smith": "откладывает молот",
    "guard": "перехватывает копьё",
    "ordinator": "перехватывает копьё",
    "mercenary": "поправляет ремень меча",
    "warrior": "поправляет ремень меча",
    "crusader": "поправляет ремень меча",
    "knight": "поправляет ремень меча",
    "archer": "поправляет тетиву",
    "scout": "оглядывает дорогу",
    "priest": "складывает ладони",
    "monk": "складывает ладони",
    "healer": "складывает ладони",
    "pilgrim": "поправляет посох",
    "mage": "стряхивает с рукава пыль",
    "sorcerer": "стряхивает с рукава пыль",
    "enchanter": "стряхивает с рукава пыль",
    "battlemage": "стряхивает с рукава пыль",
    "farmer": "вытирает руки о штанину",
    "miner": "вытирает руки о штанину",
    "fisherman": "сматывает леску",
    "bard": "перебирает струны",
    "thief": "оглядывается через плечо",
    "rogue": "оглядывается через плечо",
    "assassin": "оглядывается через плечо",
    "agent": "оглядывается через плечо",
    "noble": "поправляет перстень",
    "pauper": "кутается в тряпьё",
    "beggar": "кутается в тряпьё",
    "slave": "опускает глаза",
}
_GESTURE_DEFAULT = "переминается с ноги на ногу"


def class_gesture(npc_class: str) -> str:
    """Жест, который этому человеку правда доступен."""
    key = (npc_class or "").strip().lower()
    for name, g in _CLASS_GESTURE.items():
        if name in key:
            return g
    return _GESTURE_DEFAULT


# Напоминание ПОСЛЕ истории и после реплики игрока — самое последнее, что
# модель читает перед тем, как отвечать.
#
# Всё остальное руководство лежит в системном промпте, то есть ДО разговора.
# Мы уже натыкались на это с другой стороны: когда неизменные правила уехали
# в начало ради кеша, ответ ускорился вдвое, но «жди здесь» просело с 3/3 до
# 0/3 — модель держит конец промпта крепче начала. Тогда лечили хвостом
# системного промпта; но и он всё равно раньше истории.
#
# Держим коротким. Это единственный текст, который пересчитывается всегда, и
# он же перебивает всё сказанное выше — длинный список тут начнёт спорить с
# самим руководством.
POST_HISTORY = (
    "---\n"
    "Отвечай на ПОСЛЕДНЮЮ реплику игрока. По-русски, своими словами, в "
    "характере.\n"
    "Сначала САМА РЕЧЬ, потом строки EMOTION, ACTION, TARGET, DISP.\n"
    "ACTION: none — обычный случай. Другое ставь, только если персонаж прямо "
    "сейчас так и поступает."
)
...
def _build_system_prompt(
    ...
    # Слабой модели — короткий промпт. Она не удержит двадцать две тысячи
    # символов: путает формат и теряет теги, а на разбор такого промпта тратит
    # больше времени, чем на сам ответ.
    if lite:
        # ПОРЯДОК ЗДЕСЬ — ЭТО СКОРОСТЬ, а не оформление.
        #
        # llama.cpp (а значит LM Studio и Ollama) переиспользует вычисления для
        # СОВПАДАЮЩЕГО НАЧАЛА промпта: одинаковый префикс не считается заново.
        # Пока промпт начинался с имени и отношения NPC, он менялся с третьей
        # строки — общего начала у двух собеседников было 257 символов из 5572,
        # и 95% руководств пересчитывалось на КАЖДУЮ реплику. Это и съедало
        # секунды, которые игрок видел как задержку.
        #
        # Теперь сначала всё неизменное (правила, правила мира, схема ответа),
        # и только в конце — кто этот человек. Менять порядок нельзя.
        who = [
            f"ТЫ: {npc_name}, {npc_race}, {npc_class}."
            + (f" Состоишь в: {npc_faction}." if npc_faction else "")
            + f" Место: {location}.",
        ]
        if disposition is not None:
            band = ("ненавидит тебя" if disposition <= 20 else
                    "настороженно" if disposition <= 40 else
                    "спокойно" if disposition <= 60 else
                    "по-доброму" if disposition <= 80 else "с доверием")
            who.append(f"К игроку относишься {band} ({disposition} из 100).")
        if life_facts:
            who.append("О себе: " + "; ".join(life_facts[:2]) + ".")
        # Формулировка тут решает всё. «Твой жест, если он нужен» большая
        # модель прочла как «ставь жест всегда»: 25 реплик из 28 открывались
        # им, и разных начал осталось 9 из 28 — речь стала однообразной.
        # Жест — редкая возможность, а не обязанность.
        who.append(f"Обычно жест не нужен — говори словами. Если он всё же "
                   f"к месту, тебе подошёл бы *{class_gesture(npc_class)}*.")

        # Напоминание в самом хвосте. Перестановка неизменного вперёд ускорила
        # ответ вдвое (9.5 -> 4.6 с), но уронила соблюдение формата: «жди
        # здесь» с 3/3 до 0/3. Модель сильнее держит КОНЕЦ промпта, а правила
        # уехали в начало.
        #
        # Хвост и так пересчитывается на каждого NPC, так что эти две строки
        # ничего не стоят по времени. Напоминание нарочно двустороннее: когда
        # я усилил только «ставь действие», модель начала цеплять follow к
        # вопросу про дорогу.
        # Перечислять действия в хвосте НЕЛЬЗЯ: замер показал, что модель
        # начинает их ставить куда попало — на вопрос «где таможня» отвечала
        # ACTION: follow. Напоминаем только про строки и про то, что по
        # умолчанию действия нет.
        # СНАЧАЛА РЕПЛИКА. Прежний хвост начинался с «не забудь строки тегов»,
        # и модель однажды выдала одни теги без единого слова — в игре это
        # немой персонаж. Сперва напоминаем про речь, теги следом.
        tail = ("Сначала САМА РЕПЛИКА — живые слова персонажа, без них ответ "
                "бессмысленен. Потом строки EMOTION, ACTION, TARGET, DISP.\n"
                "ACTION: none — обычный случай. Другое ставь, только если "
                "персонаж прямо сейчас так и поступает.\n"
                # Правила тут мало: модель, уже согласившаяся идти, на просьбу
                # «стой тут» отвечала «я с тобой пошёл» и снова ставила follow.
                # Слабой модели нужен ПРИМЕР ровно этого перехода.
                "Отвечай на ПОСЛЕДНЮЮ реплику игрока, а не на прошлые:\n"
                "    раньше сказал «иду за тобой», а теперь просят «стой тут»\n"
                "    -> «Ладно, жду.» и ACTION: wait_here (НЕ follow)\n"
                "DISP не спорит с чувством: злишься или брезгуешь — DISP "
                "ноль или меньше; радуешься — ноль или больше.")
        # ПОРЯДОК ХВОСТА — ЭТО ТОЖЕ СКОРОСТЬ. Напоминание неизменно, но стоит
        # ПОСЛЕ переменного блока про NPC, а значит пересчитывается вместе с
        # ним для каждого собеседника. Замер: у двух разных NPC расходится 11%
        # промпта, и разбор этого хвоста стоит больше секунды из трёх.
        #
        # ПРОВЕРЕНО И ОТКЛОНЕНО. Перестановка правда ускоряет: изменяемая
        # часть падает с 678 знаков до 193, медиана реплики с 2.7 с до 1.7 с.
        # Но точность рушится, и заметно — по 84 сцены на вариант:
        #     хвост в конце   81 из 84, спотыкается только на «жди здесь»
        #     хвост впереди   73 из 84, спотыкается на ПЯТИ разных сценах
        # Восемь баллов при погрешности около трёх — потеря настоящая. Модель
        # держит конец промпта крепче начала, и стоит унести оттуда
        # напоминание, как рассыпаются просьбы идти, ждать и звать стражу.
        # Полторы секунды того не стоят. Переключатель оставлен, чтобы не
        # проверять это заново на новой модели с нуля.
        if os.environ.get("MWAI_TAIL_FIRST", "0") != "0":
            bits = [LITE_RULES, house_rules(), LITE_SCHEMA, tail, "\n".join(who)]
        else:
            bits = [LITE_RULES, house_rules(), LITE_SCHEMA, "\n".join(who), tail]
...
# Ремарка звёздочками — «*вздыхает*», «*вытирает тряпкой стойку*».
#
# Раньше мы их ВЫРЕЗАЛИ здесь: промпт запрещает описания поз, а модель всё
# равно писала их в трети реплик. Борьба была проигранной и вредной — жест
# живой, а выбрасывался вместе с потраченным на него временем.
#
# Теперь ремарка ОСТАЁТСЯ в тексте: игрок видит её в субтитрах. Не озвучивает
# её синтезатор — см. strip_stage_directions в tts_queue.
...
def _despine(line: str) -> str | None:
    ...
    речи в ней нет.
    """
    # Ремарку НЕ вырезаем — она нужна в субтитрах.
    #
    # И строку из ОДНОГО жеста тоже оставляем: модель ставит его отдельной
    # строкой перед речью, а если такие строки выбрасывать, жест исчезает при
    # вполне нормальной реплике. Замер это и показал: 19 жестов в сыром выводе
    # и ноль в разобранном. Проверка «а есть ли речь вообще» делается ниже, по
    # собранному ответу целиком.
    s = _MARKER_RE.sub("", line).strip()
    if not s:
        return "" if line.strip() == "" else None
    if _STAGE_RE.fullmatch(s):
        return s
...
def _parse_response(raw_text: str) -> tuple[str, str, str, str, int, int, str, str]:
    ...
    # Ответ из ОДНИХ жестов речью не является: персонаж открыл бы рот и
    # промолчал. Отдаём пустоту — её подхватит переспрос в агенте.
    if dialogue and not _STAGE_RE.sub("", dialogue).strip():
        dialogue = ""
...
class LoreAgent:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        # Короткий промпт для слабых моделей. По умолчанию включается сам для
        # всего, что крутится дома: полный промпт — 22 тысячи символов, и
        # 7-8-миллиардная модель на нём путает формат и теряет теги.
        # Явная настройка prompt_profile (lite|full) перебивает догадку.
        prov = str(provider_cfg.get("provider") or "").lower()
        profile = str(provider_cfg.get("prompt_profile")
                      or config.get("prompt_profile") or "").lower()
        if profile in ("lite", "full"):
            self._lite = profile == "lite"
        else:
            self._lite = prov in ("ollama", "llamacpp", "lmstudio", "local")

        # ТЕМПЕРАТУРА ДЛЯ ДОМАШНЕЙ МОДЕЛИ. Замерено на gigachat, по четыре
        # круга семи сцен:
        # Полный перебор, по четыре круга семи сцен:
        #     0.1 — 28/28, повтор 71%, ДОСЛОВНЫХ повторов 14
        #     0.3 — 26/28, повтор 47%, дословных 2
        #     0.4 — 25/28, повтор 46%, дословных 0
        #     0.5 — 27/28, повтор 41%, дословных 1
        #     0.6 — 26/28, повтор 38%, дословных 0
        #     0.7 — 24/28, повтор 34%, дословных 0
        #     0.8 — 20/21, повтор 35%, дословных 0
        #
        # Точность от 0.3 до 0.8 колеблется в пределах 86-96%, а один и тот же
        # промпт на 0.8 давал от 81% до 95% просто от прогона к прогону — то
        # есть вся эта колонка ШУМ. Повторяемость же падает монотонно, без
        # исключений: это настоящая зависимость.
        #
        # Раз точность плоская, берём самую ВЫСОКУЮ приемлемую температуру, а
        # не самую низкую. Прежний выбор 0.3 опирался на сравнение двух
        # случайных чисел — ошибка, исправлена перебором.
        if self._lite and "temperature" not in provider_cfg:
            self._temperature = 0.5

        # И потолок длины. Генерация идёт по токену, поэтому она — главная
        # статья расхода времени: замер показал связь длины ответа со временем
        # почти прямую. Реплика по правилам одно-два предложения, а разрешено
        # было 300 токенов — модель иногда растекалась на весь лимит.
        ...
        logger.info(
            ...
            "короткий" if self._lite else "полный",
        )

    # Слова, по которым видно согласие. Дешёвый предварительный фильтр: гонять
    # модель второй раз на каждую реплику незачем — переспрашиваем, только
    # если реплика ПОХОЖА на «да».
    _AGREE_HINTS = (
        "ладно", "хорошо", "идём", "идем", "пошли", "веди", "иду", "пойду",
        "жду", "подожду", "буду ждать", "буду тут", "буду здесь", "останусь",
        "держу место", "договорились", "согласен", "согласна", "как скажешь",
    )

    async def _ask_which_action(self, dialogue: str, player_said: str) -> str:
        """Переспросить модель одним словом: на что она согласилась.

        Только для слабых моделей и только когда реплика похожа на согласие.
        Стоит секунду и один короткий запрос, зато убирает главную поломку
        локального режима — «сказал да, а в игре ничего».
        """
        low = dialogue.lower()
        if not any(h in low for h in self._AGREE_HINTS):
            return ""
        try:
            resp = await self.llm.complete(
                system="Ты разбираешь реплику и отвечаешь ОДНИМ словом.",
                messages=[{"role": "user", "content":
                           f"Игрок сказал: {player_said}\n"
                           f"Персонаж ответил: {dialogue}\n\n"
                           "Персонаж согласился пойти за игроком, остаться "
                           "ждать на месте, или ни то ни другое?\n"
                           "Ответь одним словом: follow, wait_here или none."}],
                temperature=0.0,
                max_tokens=8,
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning("страховка не сработала: %s", exc)
            return ""
        word = (resp.text or "").strip().lower()
        for a in ("wait_here", "follow"):
            if a in word:
                return a
        return ""
    ...
    async def generate_response(
        ...
        # Стражник — особый случай, и не только когда его позвали. Он на
        # службе, у него есть устав и начальство, и разговор с ним идёт не как
        # с прохожим. Отдельная ветка под роль оказалась удачной мыслью — её
        # подсмотрел у соседнего мода, где стража тоже вынесена в промпт особо.
        if "guard" in npc_class.lower() or "ordinator" in npc_class.lower():
            user_parts.append(
                "ТЫ НА СЛУЖБЕ. Говоришь коротко и по делу — не грубо, а "
                "по-служебному: у тебя смена, а не беседа. Отвечаешь на вопросы "
                "о городе, дороге, законе и порядке; сплетничать и жаловаться на "
                "жизнь можешь, только если разговор к тому расположил.\n"
                "Закон — это ты, и он работает по поводу, а не по настроению: "
                "не грози тюрьмой тому, кто ничего не сделал, и не требуй "
                "штрафа за слова. Увидел преступление — другое дело.\n"
                "И помни, что ты человек в доспехе, а не устав: усталость под "
                "конец смены, ворчание на начальство, слабость к выпивке или "
                "к монете — всё это твоё, если оно в твоём характере."
            )
        ...
        # ПО УМОЛЧАНИЮ ВЫКЛЮЧЕНО. Приём известный и обычно работает: правила
        # ставят ПОСЛЕ истории разговора, чтобы модель прочла их последними.
        # У нас он не дал ничего — 84 сцены на вариант, гигачат, температура
        # 0.5:
        #     без пост-инструкций  77 из 84, медиана 9.6 с
        #     с пост-инструкциями  75 из 84, медиана 9.1 с
        # Два промаха из 84 меньше погрешности замера (около трёх). Скорость
        # тоже не изменилась — на неё этот блок не влияет вовсе.
        #
        # Причина, похоже, в том, что приём у нас уже наполовину сделан:
        # хвост-напоминание в системном промпте появился по той же логике,
        # когда правила уехали в начало ради кеша и точность просела. Он
        # стоит перед историей, но этого уже хватает.
        #
        # Включается MWAI_POSTHISTORY=1, если захочется перепроверить.
        if os.environ.get("MWAI_POSTHISTORY", "0") != "0":
            user_parts.append(POST_HISTORY)
        user_message = "\n\n".join(user_parts)
        ...
        # ПУСТАЯ РЕПЛИКА — переспрашиваем. Слабая модель иногда выдаёт одни
        # теги без единого слова, и в игре это немой персонаж: окно открылось,
        # а сказать нечего. Одна повторная попытка дешевле такого провала.
        if getattr(self, "_lite", False) and not dialogue.strip():
            logger.warning("модель ответила одними тегами — переспрашиваю")
            try:
                retry = await self.llm.complete(
                    system=system_prompt,
                    messages=messages + [{
                        "role": "user",
                        "content": "Ты не сказал ни слова — были одни теги. "
                                   "Ответь заново: сначала реплика персонажа, "
                                   "потом строки тегов."}],
                    temperature=self._temperature,
                    max_tokens=self._max_tokens,
                )
                again = _parse_response(retry.text)
                if again[0].strip():
                    dialogue, emotion = again[0], again[1]
                    if again[2] != "none":
                        action, target = again[2], again[3]
                    total_tokens += retry.tokens_in + retry.tokens_out
            except Exception as exc:  # noqa: BLE001
                logger.warning("переспрос не удался: %s", exc)

        # СТРАХОВКА ДЛЯ СЛАБЫХ МОДЕЛЕЙ. Домашняя модель соглашается словами
        # («Ладно, жду здесь», «Держу место»), а тег ставить забывает — и в
        # игре не меняется ничего: игрок уходит, спутник стоит.
        #
        # Переспрашиваем ровно в этом случае: реплика похожа на согласие, а
        # действия нет. Вопрос короткий, отвечать одним словом — секунда.
        if getattr(self, "_lite", False) and action == "none" and dialogue:
            guessed = await self._ask_which_action(dialogue, player_input)
            if guessed:
                action = guessed
                total_tokens += 40    # примерная цена переспроса
                logger.info("страховка: модель согласилась словами, "
                            "проставлено действие %s", guessed)
