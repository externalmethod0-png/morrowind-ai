...
class LoreAgent:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        # круга семи сцен:
        #     0.8 — 17-20 из 21, реплики повторяются на 33%  (живо, но врёт формат)
        #     0.3 — 26 из 28,    повтор 47%                  (баланс)
        #     0.1 — 28 из 28,    повтор 70%                  (точно, но как автоответчик)
        #
        # Берём 0.3: последние семь процентов точности стоят того, что персонаж
        # начинает отвечать одно и то же слово в слово, а весь мод про живых
        # людей. Формат добирают страховки — переспрос на пустую реплику и на
        # потерянное действие. Явная настройка в конфиге сильнее этой.
