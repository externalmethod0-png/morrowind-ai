...
LITE_SCHEMA = """Отвечай ровно тремя строками после реплики и ничего не добавляй.
...
А вот пример, когда персонаж СОГЛАСИЛСЯ идти за игроком:
...
И когда согласился остаться на месте:
...
ACTION — одно слово из семи:
    none        обычный разговор, ничего не происходит
    follow      согласился идти за игроком
    wait_here   согласился остаться и ждать
    flee        убегает
    attack      бросается в драку
    trade       открывает торговлю
    callguards  зовёт стражу
...
def _build_system_prompt(
    ...
    if lite:
        ...
        if disposition is not None:
            ...
            bits.append(f"К игроку относишься {band} ({disposition} из 100).")
        if life_facts:
            bits.append("О себе: " + "; ".join(life_facts[:2]) + ".")
        bits.append(house_rules())
        bits.append(LITE_SCHEMA)
