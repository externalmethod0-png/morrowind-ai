...
# Адрес сервера моделей. По умолчанию LM Studio, но своя сборка llama-server
# (например форк PrismML для Bonsai) слушает другой порт — задаётся
# переменной MWAI_BENCH_URL.
...
SCENES = [
    ...
    },
    {
        ...
        # callguards тут ЗАКОНЕН — исправление моего же ожидания. Игрок
        # предлагает заказное убийство, а это настоящее преступление, за
        # которое звать стражу правильно (в полном промпте это прямо
        # разрешено). Прежний список оставлял персонажу выбор только между
        # согласием на убийство и молчанием.
        "want_action": {"poison", "none", "callguards"},
        "need_target_if": "poison",
        "why": "согласие допустимо (тогда назови жертву), отказ и донос — тоже",
    },
    {
        "name": "труп у ног",
        ...
        "forbid_words": ("не видел", "не знаю никого", "во снах", "какое тело",
                         ...
                         "кто-то другой"),
        # follow тут ЗАКОНЕН, и это исправление моей же ошибки в ожиданиях.
        # Игрок говорит «идём дальше искать» — это прямая просьба идти вместе,
        # и согласие на неё правильно. Сцена написана проверять другое: не
        # станет ли стражник отрицать, что убил Фаргота сам, и не выдумает ли
        # чужого убийцу. За это отвечает forbid_words, он и остаётся строгим.
        "want_action": {"none", "attack", "callguards", "threaten", "flee",
                        "defend", "follow"},
...
async def run_model(model: str, raw_log: list, rounds: int = 1) -> dict:
    ...
    agent._temperature = float(os.environ.get("MWAI_BENCH_TEMP", "0.8"))
